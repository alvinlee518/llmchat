...
public class WeaviateFilterMapper {
    ...
    private static WhereFilter mapGreaterThan(IsGreaterThan isGreaterThan, String... metadataKeys) {
        return buildWhereFilter(Operator.GreaterThan, isGreaterThan.key(), isGreaterThan.comparisonValue(), metadataKeys);
    }

    private static WhereFilter mapGreaterThanOrEqual(IsGreaterThanOrEqualTo isGreaterThanOrEqualTo, String... metadataKeys) {
        return buildWhereFilter(Operator.GreaterThanEqual, isGreaterThanOrEqualTo.key(), isGreaterThanOrEqualTo.comparisonValue(), metadataKeys);
    }
    ...
    private static WhereFilter mapLessThanOrEqual(IsLessThanOrEqualTo isLessThanOrEqualTo, String... metadataKeys) {
        return buildWhereFilter(Operator.LessThanEqual, isLessThanOrEqualTo.key(), isLessThanOrEqualTo.comparisonValue(), metadataKeys);
    }
    ...
    private static WhereFilter mapNotIn(IsNotIn isNotIn, String... metadataKeys) {
        WhereFilter inFilter = buildWhereFilter(Operator.ContainsAny, isNotIn.key(), isNotIn.comparisonValues().toArray(), metadataKeys);
    ...
    private static void handleListValue(WhereFilter.WhereFilterBuilder builder, List<?> list, boolean isMetadataKey) {
        ...
        if (firstValue instanceof Number) {
            ...
            builder.valueDate(list.stream().map(item -> Date.from(((LocalDateTime) item).atZone(ZoneId.systemDefault()).toInstant())).toArray(Date[]::new));
