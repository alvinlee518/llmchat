...
public class StructuredDataLoader {
    ...
    private static List<TextSegment> parseCSV(UrlSource source) throws IOException {
        ...
        for (CSVRecord record : records) {
            ...
            TextSegment segment = TextSegment.from(completion, Metadata.from(Map.of(TextSegmentLoader.PROMPT, prompt, TextSegmentLoader.INDEX, index)));
    ...
    private static List<TextSegment> parseXLSAndXLSX(UrlSource source, String fileExtension) throws IOException {
        ...
        try (Workbook workbook = StringUtils.equalsIgnoreCase("xlsx", fileExtension) ? new XSSFWorkbook(source.inputStream()) : new HSSFWorkbook(source.inputStream())) {
            ...
            for (int rowNum = rowStart; rowNum < rowEnd; rowNum++) {
                ...
                TextSegment segment = TextSegment.from(completion, Metadata.from(Map.of(TextSegmentLoader.PROMPT, prompt, TextSegmentLoader.INDEX, rowNum)));
                segmentList.add(segment);
            }
        }
        return segmentList;
